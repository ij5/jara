...
pub fn report_detect(
    ...
    for (class_index, bboxes_for_class) in bboxes.iter().enumerate() {
        for b in bboxes_for_class.iter() {
            println!("{}: {:?}", super::coco_classes::NAMES[class_index], b);
            ...
            if legend_size > 0 {
                ...
                let legend = format!(
                    "{}   {:.0}%",
                    super::coco_classes::NAMES[class_index],
                    100. * b.confidence
                );
...
pub fn report_pose(
    ...
    confidence_threshold: f32,
    nms_threshold: f32,
) -> Result<(DynamicImage, Vec<Bbox<Vec<KeyPoint>>>)> {
    let pred = pred.to_device(&Device::Cpu)?;
    let (pred_size, npreds) = pred.dims2()?;
    ...
    for b in bboxes.iter() {
        ...
        for (i, kp) in b.data.iter().enumerate() {
            if kp.mask < 0.6 {
                continue;
            }
    ...
    Ok((DynamicImage::ImageRgb8(img), bboxes.to_vec()))
}
...
pub fn run<T: Task>(
    tx: mpsc::Sender<(DynamicImage, Vec<Bbox<Vec<KeyPoint>>>)>,
    mut camera: Camera,
) -> anyhow::Result<()> {
    let device = super::device::device(true)?;
    // Create the model and load the weights from the file.
    let mut args = Args::parse();
    args.task = YoloTask::Pose;
    args.which = Which::N;
    ...
    while let Ok(frame) = camera.frame() {
        ...
        let original_image = original_image.flipv();
        println!("processing...");
        let (width, height) = {
        ...
        // let image_t = image_t.flipv();
        tx.send((image_t, bboxes))?;
...
pub fn setup(camera: Camera) -> mpsc::Receiver<(DynamicImage, Vec<Bbox<Vec<KeyPoint>>>)> {
    let (tx, rx) = mpsc::channel();
    let _ = std::thread::spawn(move || {
        run::<YoloV8Pose>(tx, camera).unwrap();
    });
    rx
}
